...
public class NetworkManager {
    ...
    private let headers: HTTPHeaders = {
        let tokenHeader: HTTPHeader = .init(name: "Authorization", value: "Bearer {토큰값}") // MARK: - Github 토큰값 넣기
    ...
    func fetchData<T: Decodable>(url: String, method: HTTPMethod, parameters: Parameters?) async -> Result<T, NetworkError> {
        ...
        let result = await self.session.request(url, method: method, parameters: parameters, headers: headers).serializingData().response
